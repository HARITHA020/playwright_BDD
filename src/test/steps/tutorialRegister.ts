...
Then("user should see Logout option", async function (this:CustomWorld) {
    await this.page.locator("//span[text()='My Account']").click();
    await expect(this.page.locator("//ul[@class='dropdown-menu dropdown-menu-right']//a[text()='Logout']")).toBeVisible();
});
Then("user should see email already exists message", async function (this:CustomWorld) {
    await expect(this.page.locator("//div[contains(text(),'Warning: E-Mail Address is already registered!')]")).toBeVisible();
});